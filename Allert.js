...
class Allert {
	...
	static displayMessage(messageConfig){
		...
		setTimeout( () => {
			div.style.opacity = 0
		} , ( messageConfig.time || this.config.time ) - Number((messageConfig.style?messageConfig.style.transitionDuration:this.config.style.transitionDuration).split('m')[0].split('s')[0]) )
		this.stack.push(div)
		this.adjustTop()
	}
}
